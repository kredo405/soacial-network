import React from 'react';
import messege from './css/messege.module.css';
import OneMessage from './oneMessage/oneMessage';






function Messege(props) {
  return (
    <div>
      <div className={messege.messege}>
        <div className={messege.messege_header}>
          <div><img className={messege.message_avatar} src={props.messageImage} /> {props.messageName}</div>
          <div>{props.status}</div>
        </div>
        <div className={messege.correspondence}>
          <OneMessage image={require('./img/4.jpg')} text='Привет как дела??' />
          <OneMessage image={require('./img/avatar.png')} text='Привет, норм, ты как???' />
        </div>
        <div className = {messege.input_message}> 
        <div className={messege.input_group}>
  <textarea className={messege.input}></textarea>
  <button className={messege.input_addon}><svg width="2em" height="2em" viewBox="0 0 16 16" class="bi bi-reply-all-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path d="M8.079 11.9l4.568-3.281a.719.719 0 0 0 0-1.238L8.079 4.1A.716.716 0 0 0 7 4.719V6c-1.5 0-6 0-7 8 2.5-4.5 7-4 7-4v1.281c0 .56.606.898 1.079.62z"/>
  <path fill-rule="evenodd" d="M10.868 4.293a.5.5 0 0 1 .7-.106l3.993 2.94a1.147 1.147 0 0 1 0 1.946l-3.994 2.94a.5.5 0 0 1-.593-.805l4.012-2.954a.493.493 0 0 1 .042-.028.147.147 0 0 0 0-.252.496.496 0 0 1-.042-.028l-4.012-2.954a.5.5 0 0 1-.106-.699z"/>
</svg></button>
</div>
        </div>
      </div>
    </div>
  );
}

export default Messege;